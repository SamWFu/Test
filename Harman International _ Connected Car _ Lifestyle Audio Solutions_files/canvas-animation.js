...
(function() {
    ...
    function initHeader() {
        ...
        largeHeader = document.getElementById('block-views-solutions-block');


        canvas = document.getElementById('solution-canvas');
        canvas.width = width;
    ...
    function animate() {
        if (animateHeader) {
            ...
            for (var i in points) {
                ...
                if (Math.abs(getDistance(target, points[i])) < 500000) {
                    points[i].active = 0.3;
    ...
    function shiftPoint(p) {
        TweenLite.to(p, 1 + 1 * Math.random(), {
            x: p.originX - 90 + Math.random() * 100,
            y: p.originY - 90 + Math.random() * 100,
            ease: Circ.easeInOut,
            onComplete: function() {
                shiftPoint(p);
            }
